...
E.CreatedOn AS CreatedOnDate,convert(nvarchar(20),M.EmployeeAssignDateTime,101) AS WorkshopAssignDate,convert(nvarchar(20),M.DispatchDate,100)as DispatchDate,case M.DispatchStatus when '1' then 'Dispatch' else 'Pendin
g' end as DispatchStatus,ISNULL(INC.InName,'') AS DispatchedBy
FROM Estimate E      
...
WHERE E.IsApproved=1 AND m.PSId=@PsId AND E.ModifyOn >= @StartDate and E.ModifyOn <= @EndDate AND M.PurchaseEmpID = @EmpId AND E.AcaId in (select * from [dbo].[Split](@Acaid,','))      
