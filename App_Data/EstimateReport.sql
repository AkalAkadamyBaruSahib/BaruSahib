...
SELECT distinct Estimate.EstId,Z.ZoneName AS Zone,A.AcaName AS Academy,MT.MatTypeName as MaterialType,Material.MatName AS Material,
                ...
                WHEN (dbo.fnMaterialReceivedQuantity(ER.Sno)>0) AND (ER.Qty=ER.PurchaseQty) AND (dbo.fnMaterialDispatchQuantity(ER.Sno)!=ER.Qty)  THEN 'InStore'       
				WHEN (ER.PurchaseQty>0) AND (dbo.fnMaterialReceivedQuantity(ER.Sno)=0)  THEN 'Purchased'       
                WHEN (dbo.fnMaterialReceivedQuantity(ER.Sno)!=ER.Qty) AND (ER.Qty!=ER.PurchaseQty) AND (dbo.fnMaterialDispatchQuantity(ER.Sno)!=ER.Qty) THEN 'Pending' 
...
--select * from EstimateAndMaterialOthersRelations